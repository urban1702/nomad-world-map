...
## v1.4
- PHP 8 compatibility fixes for deprecated/removed function calls and stricter version comparison checks
- Improved Google Maps loading and script dependency order for frontend and admin map editor
- Fixed settings handling for Google API keys and enabled SnazzyMap style support in admin/dev maps
- Added admin-side SnazzyMap style loading/parsing for dev map previews
- Hardened admin SQL queries with prepared statements and safer input handling
- Improved settings sanitization and request unslashing to reduce malformed input issues

## v1.3.2
- Added the option to style the frontend map via Snazzy Maps
- Fixed wordpress css style support for the existing admin tables
- Fixed FAQ page
